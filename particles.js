document.addEventListener("DOMContentLoaded", function () {
  ...
  // Handle window resize (preserve particles; adjust positions proportionally)
  ...
  window.addEventListener("resize", function () {
    ...
    canvas.width = newW;
    canvas.height = newH;
    // Scale existing particle positions so layout feels stable
    particles.forEach((p) => {
      p.x *= scaleX;
      p.y *= scaleY;
    });
    // If area change is large (>20%), gently nudge density (add/remove) without full regeneration
